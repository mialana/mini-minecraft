...
BlockType Terrain::getBlockAt(int x, int y, int z) const {
  if (hasChunkAt(x, z)) {
    ...
    glm::vec2 chunkOrigin =
        glm::vec2(floor(x / 16.f) * 16, floor(z / 16.f) * 16);
...
void Terrain::multithreadedWork(glm::vec3 currPlayerPos,
                                glm::vec3 prevPlayerPos, float dt) {
  m_chunkTimer += dt;
...
QSet<long long> Terrain::borderingZone(glm::ivec2 coords, int radius,
                                       bool atEdge) {
...
void Terrain::tryExpansion(glm::vec3 pos, glm::vec3 prevPos) {
  ...
  glm::ivec2 prev(64.f * floor(prevPos.x / 64.f),
                  64.f * floor(prevPos.z / 64.f));
...
void Terrain::createBDWorker(long long zone) {
  ...
  BDWorker *worker =
      new BDWorker(x, z, toDo, &m_blockDataChunks, &m_blockDataChunksLock);
...
void Terrain::createVBOWorker(Chunk *chunk) {
  VBOWorker *werk =
      new VBOWorker(chunk, &m_vboDataChunks, &m_VBODataChunksLock);
...
void Terrain::setBlockAt(int x, int y, int z, BlockType t) {
  if (hasChunkAt(x, z)) {
    ...
    c->setBlockAt(static_cast<unsigned int>(x - chunkOrigin.x),
                  static_cast<unsigned int>(y),
                  static_cast<unsigned int>(z - chunkOrigin.y), t);
  } else {
    throw std::out_of_range("Coordinates " + std::to_string(x) + " " +
                            std::to_string(y) + " " + std::to_string(z) +
                            " have no Chunk!");
...
void Terrain::setBiomeAt(int x, int z, glm::vec4 b) {
  if (hasChunkAt(x, z)) {
    ...
    c->setBiomeAt(static_cast<unsigned int>(x - chunkOrigin.x),
                  static_cast<unsigned int>(z - chunkOrigin.y), b);
  } else {
    throw std::out_of_range("Coordinates " + std::to_string(x) +
                            std::to_string(z) + " have no Chunk!");
  }
    ...
    for (int z = minZ; z < maxZ; z += 16) {
      if (hasChunkAt(x, z)) {
        ...
        if (currChunk->hasVBOData && currChunk->hasBinded) {
          shaderProgram->setModelMatrix(
              glm::translate(glm::mat4(), glm::vec3(x, 0, z)));
  ...
  if (mobsToRespawn.size() > 0) {
    ...
    if (availableChunks.size() > 0) {
      for (Mob *mob : mobsToRespawn) {
        int randomChunk =
            Biome::getRandomIntInRange(0, availableChunks.size() - 1);
...
void Terrain::loadNewChunks(glm::vec3 currPos) {
  for (const glm::ivec2 &p : directionHelper) {
    if (!hasChunkAt(currPos.x + p[0], currPos.z + p[1])) {
      glm::ivec2 currChunkOrigin =
          16 * glm::ivec2(glm::floor(glm::vec2(currPos.x, currPos.z) / 16.f));

      glm::ivec2 newChunkOrigin = currChunkOrigin + p;

      Chunk *newChunk =
          instantiateChunkAt(newChunkOrigin[0], newChunkOrigin[1]);
      newChunk->createVBOdata();
    }
  }
}

std::pair<float, BiomeEnum>
Terrain::blendMultipleBiomes(glm::vec2 xz, float forestH, float mountH,
  ...
  float h = (biomeWts.x * mountH) + (biomeWts.y * hillH) +
            (biomeWts.z * forestH) + (biomeWts.w * islandH);
  return std::pair(h, b);
}
