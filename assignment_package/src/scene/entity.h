...
struct InputBundle {
  ...
  InputBundle()
      : wPressed(false), aPressed(false), sPressed(false), dPressed(false),
        ePressed(false), fPressed(false), qPressed(false), spacePressed(false),
        mouseX(0.f), mouseY(0.f), flightMode(true), onGround(false),
        isMoving(false), inLiquid(false), underWater(false), underLava(false),
        inThirdPerson(false), isZombie(false), isPig(false),
...
public:
  ...
  virtual bool gridMarch(glm::vec3 rayOrigin, glm::vec3 rayDirection,
                         float *out_dist, glm::ivec3 *out_blockHit,
                         Terrain &terrain, BlockType *out_type = nullptr);
