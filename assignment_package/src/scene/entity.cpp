...
Entity::Entity(OpenGLContext *context) : Entity(glm::vec3(0, 0, 0), context) {}

Entity::Entity(glm::vec3 pos, OpenGLContext *context)
    : m_inputs(InputBundle()), infAxis(-1), m_velocity(glm::vec3(0, 0, 0)),
      m_acceleration(glm::vec3(0, 0, 0)), m_forward(0, 0, -1), m_right(1, 0, 0),
      m_up(0, 1, 0), m_position(pos), m_geom3D(context) {}

Entity::Entity(const Entity &e, OpenGLContext *context)
    : m_inputs(e.m_inputs), infAxis(e.infAxis), m_velocity(e.m_velocity),
      m_acceleration(e.m_acceleration), m_forward(e.m_forward),
      m_right(e.m_right), m_up(e.m_up), m_position(e.m_position),
      m_geom3D(context) {}

Entity::~Entity() {}

void Entity::moveAlongVector(glm::vec3 dir) { m_position += dir; }

void Entity::moveForwardLocal(float amount) {
  m_position += amount * m_forward;
}

void Entity::moveRightLocal(float amount) { m_position += amount * m_right; }

void Entity::moveUpLocal(float amount) { m_position += amount * m_up; }
...
void Entity::rotateOnForwardGlobal(float degrees) {
  float rad = glm::radians(degrees);
  m_forward = glm::vec3(glm::rotate(glm::mat4(), rad, glm::vec3(0, 0, 1)) *
                        glm::vec4(m_forward, 0.f));
  m_right = glm::vec3(glm::rotate(glm::mat4(), rad, glm::vec3(0, 0, 1)) *
                      glm::vec4(m_right, 0.f));
  m_up = glm::vec3(glm::rotate(glm::mat4(), rad, glm::vec3(0, 0, 1)) *
                   glm::vec4(m_up, 0.f));
}

void Entity::rotateOnRightGlobal(float degrees) {
  float rad = glm::radians(degrees);
  m_forward = glm::vec3(glm::rotate(glm::mat4(), rad, glm::vec3(1, 0, 0)) *
                        glm::vec4(m_forward, 0.f));
  m_right = glm::vec3(glm::rotate(glm::mat4(), rad, glm::vec3(1, 0, 0)) *
                      glm::vec4(m_right, 0.f));
  m_up = glm::vec3(glm::rotate(glm::mat4(), rad, glm::vec3(1, 0, 0)) *
                   glm::vec4(m_up, 0.f));
}

void Entity::rotateOnUpGlobal(float degrees) {
  float rad = glm::radians(degrees);
  m_forward = glm::vec3(glm::rotate(glm::mat4(), rad, glm::vec3(0, 1, 0)) *
                        glm::vec4(m_forward, 0.f));
  m_right = glm::vec3(glm::rotate(glm::mat4(), rad, glm::vec3(0, 1, 0)) *
                      glm::vec4(m_right, 0.f));
  m_up = glm::vec3(glm::rotate(glm::mat4(), rad, glm::vec3(0, 1, 0)) *
                   glm::vec4(m_up, 0.f));
}
...
void Entity::constructSceneGraph(QJsonArray data) {
  ...
  for (auto i = data.begin(), end = data.end(); i != end; i++) {
    ...
    if (obj["nodeType"].toString() == "translation") {
      ...
      glm::vec3 scale =
          MyGL::convertQJsonArrayToGlmVec3(obj["scale"].toArray());
  ...
  if (m_inputs.inThirdPerson) {
    if (m_inputs.isMoving) {
      if (nodePointerMap["LeftArmR"] != nullptr && !m_inputs.isZombie) {
        (static_cast<RotateNode *>(nodePointerMap["LeftArmR"]))->degrees =
            maxAngle * glm::sin(m_timer * 10.f + M_PI);
      }
      if (nodePointerMap["RightArmR"] != nullptr && !m_inputs.isZombie) {
        (static_cast<RotateNode *>(nodePointerMap["RightArmR"]))->degrees =
            maxAngle * glm::sin(m_timer * 10.f);
      }
      if (nodePointerMap["LeftLegR"] != nullptr) {
        (static_cast<RotateNode *>(nodePointerMap["LeftLegR"]))->degrees =
            maxAngle * glm::sin(m_timer * 10.f);
      }
      if (nodePointerMap["RightLegR"] != nullptr) {
        (static_cast<RotateNode *>(nodePointerMap["RightLegR"]))->degrees =
            maxAngle * glm::sin(m_timer * 10.f + M_PI);
      }
    } else {
      if (nodePointerMap["LeftArmR"] != nullptr &&
          glm::abs((static_cast<RotateNode *>(nodePointerMap["LeftArmR"]))
                       ->degrees &&
                   !m_inputs.isZombie) >= 3.f) {
        (static_cast<RotateNode *>(nodePointerMap["LeftArmR"]))->degrees =
            maxAngle * glm::sin(m_timer * 10.f + M_PI);
      ...
      if (nodePointerMap["RightArmR"] != nullptr &&
          glm::abs((static_cast<RotateNode *>(nodePointerMap["RightArmR"]))
                       ->degrees &&
                   !m_inputs.isZombie) >= 3.f) {
        (static_cast<RotateNode *>(nodePointerMap["RightArmR"]))->degrees =
            maxAngle * glm::sin(m_timer * 10.f);
      ...
      if (nodePointerMap["LeftLegR"] != nullptr &&
          glm::abs((static_cast<RotateNode *>(nodePointerMap["LeftLegR"]))
                       ->degrees) >= 3.f) {
        (static_cast<RotateNode *>(nodePointerMap["LeftLegR"]))->degrees =
            maxAngle * glm::sin(m_timer * 10.f);
      } else {
      ...
      if (nodePointerMap["RightLegR"] != nullptr &&
          glm::abs((static_cast<RotateNode *>(nodePointerMap["RightLegR"]))
                       ->degrees) >= 3.f) {
        (static_cast<RotateNode *>(nodePointerMap["RightLegR"]))->degrees =
            maxAngle * glm::sin(m_timer * 10.f + M_PI);
      } else {
...
void Entity::drawSceneGraph(const uPtr<Node> &currNode,
...
void Entity::computePhysics(float dT, Terrain &terrain) {
  ...
  if (!m_inputs.flightMode) {
    m_velocity =
        glm::vec3(m_velocity.x, glm::min(m_velocity.y, 0.5f), m_velocity.z);
    detectCollision(terrain);
  }

  this->moveAlongVector(m_velocity);
  m_inputs.isMoving = (glm::abs(m_velocity.x / (dT / 1000.f)) >= 3.f) ||
                      (glm::abs(m_velocity.z / (dT / 1000.f)) >= 3.f);
}
  ...
  for (int x = 0; x <= 1; x++) {
    for (int z = 2; z <= 3; z++) {
      for (int y = 4; y <= 5; y++) {
        ...
        if (gridMarch(rayOrigin, rayDirectionX, &outDistX, &outBlockHitX,
                      terrain) &&
            (outDistX < minOutDistX)) {
          minOutDistX = outDistX;
        }

        if (gridMarch(rayOrigin, rayDirectionY, &outDistY, &outBlockHitY,
                      terrain) &&
            (outDistY < minOutDistY)) {
          minOutDistY = outDistY;
        }

        if (gridMarch(rayOrigin, rayDirectionZ, &outDistZ, &outBlockHitZ,
                      terrain) &&
            (outDistZ < minOutDistZ)) {
...
bool Entity::gridMarch(glm::vec3 rayOrigin, glm::vec3 rayDirection,
                       float *out_dist, glm::ivec3 *out_blockHit,
                       Terrain &terrain, BlockType *out_type) {
  ...
  while (curr_t < maxLen) {
    ...
    if (interfaceAxis == -1) {
      throw std::out_of_range(
          "interfaceAxis was -1 after the for loop in gridMarch!");
    }
    ...
    offset[interfaceAxis] =
        glm::min(0.f, glm::sign(rayDirection[interfaceAxis]));
    ...
    if (cellType != EMPTY && !Chunk::isHPlane(cellType) &&
        !Chunk::isCross2(cellType) && !Chunk::isCross4(cellType) &&
        cellType != WATER && cellType != LAVA) {
      if (out_type) {
        *out_type = cellType;
      }
  ...
  for (int x = 0; x <= 1; x++) {
    for (int z = 2; z <= 3; z++) {
      ...
      if (terrain.getBlockAt(p) != EMPTY &&
          ...
          terrain.getBlockAt(p) != WATER && terrain.getBlockAt(p) != LAVA) {
        acc = acc || true;
      } else {
        acc = acc || false;
      }
...
void Entity::drawPath(Terrain &terrain) {
  isOnGround(terrain);
  ...
  if (this->m_inputs.onGround) {
    ...
    if (cellType != EMPTY && cellType != WATER && cellType != LAVA &&
        !Chunk::isHPlane(cellType) && !Chunk::isCross2(cellType) &&
        !Chunk::isCross4(cellType)) {

      terrain.changeBlockAt(bottomCell.x, bottomCell.y, bottomCell.z,
                            MOSS_STONE);
    }
  }
}
