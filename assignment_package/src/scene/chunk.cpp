        ...
        for (int z = 0; z < 16; ++z) {
            ...
            if (b == MOUNTAINS) {
                if (h < 120) {
                    ...
                    this->viableSpawnBlocks.push_back(glm::vec3(x, h, z));
                }
            } else if (b == HILLS) {
...
glm::vec3 Chunk::getRandomViableBlock() {
    int randomViableIndex = Biome::getRandomIntInRange(0, this->viableSpawnBlocks.size() - 1);
    return this->viableSpawnBlocks[randomViableIndex];
}

glm::ivec2 Chunk::computeLocalPos(glm::vec3 pos) {
    glm::ivec2 posXZ = glm::ivec2(floor(pos.x), floor(pos.z));
    return posXZ - this->getWorldPos();
}
