...
void Drawable::destroyVBOdata()
{
    ...
    mp_context->glDeleteBuffers(1, &m_bufVertData);
    m_idxGenerated = m_posGenerated = m_norGenerated = m_colGenerated = m_vertDataGenerated = false;
    m_count = -1;
}
...
{
    m_vertDataGenerated = true;
    // Create a VBO on our GPU and store its handle in bufVertData
    mp_context->glGenBuffers(1, &m_bufVertData);
}
...
bool Drawable::bindVertData()
{
    if(m_vertDataGenerated){
        mp_context->glBindBuffer(GL_ARRAY_BUFFER, m_bufVertData);
    }
    return m_vertDataGenerated;
}
