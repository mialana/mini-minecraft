...
Player::Player(glm::vec3 pos, const Terrain &terrain, OpenGLContext *context)
    : Entity(pos, context), m_camera(pos + glm::vec3(0, 1.5f, 0), context),
      m_thirdPersonCamera(pos + glm::normalize(m_camera.m_up) * 2.f +
                              glm::normalize(m_camera.m_forward) * -5.f,
                          context),
      m_frontViewCamera(pos + glm::normalize(m_camera.m_forward) * 5.f,
                        context),
...
void Player::tick(float dT, Terrain &terrain) {
  ...
  glm::mat4 headRotateMatrix = glm::lookAt(
      ...
      m_position + glm::vec3(0.f, 1.65f, 0.f) + m_forward, glm::vec3(0, 1, 0));
...
BlockType Player::removeBlock(Terrain *terrain) {
  ...
  if (gridMarch(rayOrigin, rayDirection, &outDist, &outBlockHit, *terrain)) {
    ...
    BlockType blockType =
        terrain->getBlockAt(outBlockHit.x, outBlockHit.y, outBlockHit.z);
    ...
    if (outType == CLOTH_1) {
      ...
      if (inventory.removeItem(currBlockType)) {
        if (infAxis == 2) {
          ...
          if (foundBlock == EMPTY || foundBlock == WATER ||
              foundBlock == LAVA) {
            terrain->setBlockAt(outBlockHit.x, outBlockHit.y,
                                outBlockHit.z - glm::sign(rayDirection.z),
                                currBlockType);
            terrain->getChunkAt(outBlockHit.x, outBlockHit.z)
                .get()
                ->destroyVBOdata();
            terrain->getChunkAt(outBlockHit.x, outBlockHit.z)
                .get()
          ...
          BlockType foundBlock = terrain->getBlockAt(
              outBlockHit.x, outBlockHit.y - glm::sign(rayDirection.y),
              outBlockHit.z);
          if (foundBlock == EMPTY || foundBlock == WATER ||
              foundBlock == LAVA) {
...
void Player::moveForwardLocal(float amount) {
  ...
  (static_cast<TranslateNode *>(nodePointerMap["BodyT"]))->translation +=
      m_forward * amount;
}
...
void Player::calculateThirdPersonCameraRotation() {
  m_thirdPersonCamera.m_position = m_camera.m_position +
                                   glm::normalize(m_camera.m_up) * 2.f +
                                   glm::normalize(m_camera.m_forward) * -5.f;
  m_thirdPersonCamera.m_forward =
      glm::normalize(m_camera.m_position - m_thirdPersonCamera.m_position);
  m_thirdPersonCamera.m_right = m_camera.m_right;
  m_thirdPersonCamera.m_up =
      -glm::cross(m_thirdPersonCamera.m_forward, m_thirdPersonCamera.m_right);
}
void Player::calculateFrontViewCameraRotation() {
  m_frontViewCamera.m_position =
      m_camera.m_position + glm::normalize(m_camera.m_forward) * 5.f;
  m_frontViewCamera.m_forward =
      glm::normalize(m_camera.m_position - m_frontViewCamera.m_position);
  m_frontViewCamera.m_right = -m_camera.m_right;
  m_frontViewCamera.m_up =
      -glm::cross(m_frontViewCamera.m_forward, m_frontViewCamera.m_right);
}
