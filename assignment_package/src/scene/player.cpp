...
void Player::tick(float dT, Terrain& terrain)
{
    ...
    glm::mat4 bodyRotateMatrix = glm::lookAt(glm::vec3(),
                                             glm::normalize(glm::vec3(m_forward.x, 0, m_forward.z)),
                                             glm::vec3(0, 1, 0));
    (static_cast<RotateNode*>(nodePointerMap["BodyR"]))->overriddenTransformMatrix = glm::inverse(
        bodyRotateMatrix);

    glm::mat4 headRotateMatrix = glm::lookAt(m_position + glm::vec3(0.f, 1.65f, 0.f),
                                             m_position + glm::vec3(0.f, 1.65f, 0.f) + m_forward,
                                             glm::vec3(0, 1, 0));
    (static_cast<RotateNode*>(nodePointerMap["HeadR"]))->overriddenTransformMatrix = glm::inverse(
        headRotateMatrix);
}
    ...
    if (gridMarch(rayOrigin, rayDirection, &outDist, &outBlockHit, *terrain, &outType)) {
        if (outType == CLOTH_1) {
            ...
            if (inventory.removeItem(currBlockType)) {
                if (infAxis == 2) {
                    BlockType foundBlock = terrain->getBlockAt(outBlockHit.x,
                                                               outBlockHit.y,
                                                               outBlockHit.z
                                                                   - glm::sign(rayDirection.z));
...
{
    m_thirdPersonCamera.m_position = m_camera.m_position + glm::normalize(m_camera.m_up) * 2.f
                                     + glm::normalize(m_camera.m_forward) * -5.f;
    ...
    m_thirdPersonCamera.m_up = -glm::cross(m_thirdPersonCamera.m_forward,
                                           m_thirdPersonCamera.m_right);
}
...
{
    std::string str("( " + std::to_string(m_position.x) + ", " + std::to_string(m_position.y) + ", "
...
{
    std::string str("( " + std::to_string(m_velocity.x) + ", " + std::to_string(m_velocity.y) + ", "
...
{
    std::string str("( " + std::to_string(m_acceleration.x) + ", " + std::to_string(m_acceleration.y)
...
{
    std::string str("( " + std::to_string(m_forward.x) + ", " + std::to_string(m_forward.y) + ", "
