...
MyGL::MyGL(QWidget *parent)
  ...
  // Connect the timer to a function so that when the timer ticks the function
  // is executed
  connect(&m_timer, SIGNAL(timeout()), this, SLOT(tick()));
  // Tell the timer to redraw 60 times per second
  m_timer.start(16);
  setFocusPolicy(Qt::ClickFocus);
  setMouseTracking(true);     // MyGL will track the mouse's movements even if a
...
void MyGL::resizeGL(int w, int h) {
  ...
  m_player.setCameraWidthHeight(static_cast<unsigned int>(w),
...
void MyGL::tick() {
  ...
  float dT =
      (QDateTime::currentMSecsSinceEpoch() - m_currMSecSinceEpoch) / 1000.f;
  ...
  if (m_player.m_inputs.underWater) {
    m_progLiquid.setPlayerPosBiomeWts(
        m_terrain.getBiomeAt(prevPlayerPos.x, prevPlayerPos.z));
  ...
  sendPlayerDataToGUI(); // Updates the info in the secondary window displaying
                         // player data
...
void MyGL::sendPlayerDataToGUI() const {
  ...
  emit sig_sendPlayerChunk(QString::fromStdString(
      "( " + std::to_string(chunk.x) + ", " + std::to_string(chunk.y) + " )"));
  emit sig_sendPlayerTerrainZone(QString::fromStdString(
      "( " + std::to_string(zone.x) + ", " + std::to_string(zone.y) + " )"));
}
...
void MyGL::paintGL() {
  ...
  for (auto &mob : m_mobs) {
    if (!mob->needsRespawn) {
      ...
      glDisable(GL_CULL_FACE);
      m_progPlayer.setModelMatrix(glm::mat4());
      mob->drawSceneGraph(mob->bodyT, glm::translate(mob->m_position),
                          m_progPlayer);
      glEnable(GL_CULL_FACE);
    }
  }
...
glm::vec4 MyGL::convertQJsonArrayToGlmVec4(QJsonArray obj) {
  return glm::vec4(obj[0].toDouble(), obj[1].toDouble(), obj[2].toDouble(),
                   obj[3].toDouble());
}

void MyGL::showRecipe() { emit sig_sendRecipeWindow(); }
